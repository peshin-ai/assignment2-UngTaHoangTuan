...
public class Car {
    ...
    private Road nextRoad(){
        ...
        int Num1;
        int Num2;
        Num1 = getCarXPosition(); //trying to find road with x position closest to this x position
        Num2 = getCarYPosition(); // trying to find road with y position closest to this y position
        int Index1 = 0;
        int Index2 =0;
        int Different1 = 10000;
        int Different2 = 10000;
        if(currentRoad.getTrafficDirection().equals("east") || currentRoad.getTrafficDirection().equals("west")) {
            for (int j = 0; j < xPositions.size(); j++) { // loops through every position
                int Difference_x = Math.abs(xPositions.get(j) - Num1);
                if (Difference_x < Different1) { // checks if difference is getting smaller
                    Index1 = j;
                    Different1 = Difference_x;
                }
            }
        }
            ...
            for (int j = 0; j < xPositions.size(); j++) { // loops through every position
                ...
                if (Difference_y < Different2) { // checks if difference is getting smaller
                    Index2 = j;
                    Different2 = Difference_y;
                }
            }
        }
        ...
        if(currentRoad.getTrafficDirection().equals("east") || currentRoad.getTrafficDirection().equals("west")){
            closestXPosition = xPositions.get(Index1);
        }
        else {
    ...
    public void move() {
        if(canMoveForward()) {
            if(road.getTrafficDirection().equals("east") || road.getTrafficDirection().equals("south")) {
                xPos += 25;
                if (getRoadCarIsOn().getSign() != null && getRoadCarIsOn().getSign().getCurrentColor().equals("green")){
                    xPos += 40;
                }else if (getRoadCarIsOn().getSign() != null && getRoadCarIsOn().getSign().getCurrentColor().equals("red")){
                    xPos += 15;
                }else {
                    xPos += 25;
                }
